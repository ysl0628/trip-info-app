      ...
      {selectedDay === "overview" ? (
        <div className="mt-6 space-y-6">
          ...
          <div className="space-y-3">
            {ITINERARY.map((day) => {
              // 使用原始 ITINERARY 来检查是否是 group tour，因为翻译后的 title 可能不包含标识
              const originalDay = ORIGINAL_ITINERARY.find(
                (d) => d.day === day.day
              );
                    ...
                    {item.timeline.map((timelineItem, idx) => (
                      ...
                      >
                        ...
                        <div className="flex-1 pb-6">
                          ...
                          {/* 交通類型：標題和內容合併顯示 */}
                          {(timelineItem.title === "交通" ||
                            timelineItem.title === "Transportation") &&
                          timelineItem.from &&
                          timelineItem.to ? (
                            <div className="space-y-2">
                              <h5 className="text-base font-bold text-stone-800">
                                ...
                                <span className="font-normal text-stone-700">
                                  {timelineItem.from} → {timelineItem.to}
                                </span>
                              </h5>
                              ...
                              timelineItem.title === "景點" ||
                              timelineItem.title === "Attractions") &&
                            !timelineItem.option ? (
                            /* 景點類型：標題和地點合併顯示 */
                            <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-start gap-4 p-4 rounded-xl bg-white border border-stone-100 shadow-sm h-full">
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-stone-500 space-y-1.5">
                      {item.accommodation.hotels.map((h, i) => {
                        ...
                        const hotelLink =
                          typeof h === "object" ? h.link : undefined;
                  ...
                  {item.accommodation.roomAssignment &&
                    item.accommodation.roomAssignment.length > 0 && (
                      <div className="flex-shrink-0 pl-4 border-l border-stone-200">
                        ...
                        <div className="space-y-1.5">
                          {item.accommodation.roomAssignment.map(
                            (room, idx) => (
                              <div
                                key={idx}
